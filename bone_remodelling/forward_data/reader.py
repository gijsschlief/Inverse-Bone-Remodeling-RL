...
def main() -> None:
    """Test the forward data reader."""
    data = forward_data_reader(
        file_path=Path(__file__).parent.parent.parent / Path("data", "raw"),
    )
    if data is None:
    ...
    serial_numbers, force_profiles, final_output_densities = data
    logger.info(f"Serial Numbers Shape: {serial_numbers.shape}")
    logger.info(f"Force Profiles Shape: {force_profiles.shape}")
    logger.info(f"Final Output Densities Shape: {final_output_densities.shape}")
