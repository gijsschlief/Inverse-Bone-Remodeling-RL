...
def compare_surrogate_model(val_logits: torch.Tensor, y_val_tensor: torch.Tensor, sample_count: int = 3, show_plot: bool = True) -> List[plt.Figure]:
    ...
    """
    if not isinstance(val_logits, torch.Tensor) or not isinstance(y_val_tensor, torch.Tensor):
        raise ValueError("Both val_logits and y_val_tensor must be torch.Tensor objects.")
    ...
    if sample_count <= 0 or sample_count > len(y_val_tensor):
        raise ValueError("sample_count must be a positive integer less than or equal to the number of validation samples.")
    ...
    for idx in random_indices:
        ...
        plot_density_matrix(actual_matrix, 'Original Density Matrix', axes[0], min_true_value, max_true_value)
        plot_density_matrix(predicted_matrix, 'Predicted Density Matrix', axes[1], min_true_value, max_true_value)
