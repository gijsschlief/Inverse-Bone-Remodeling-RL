...
class SurrogateModelLoader:
    def __init__(self, model_path: str, model_class: Type[torch.nn.Module], auto_load: bool = True) -> None:
        """
...
def load_surrogate_model(model_path: str = "data/models/trained_model.pth", model_class: Type[torch.nn.Module] = AdvancedNNSurrogateModel) -> torch.nn.Module:
    """
    ...
    if not path.is_absolute():
        code_dir = Path(__file__).resolve().parent.parent.parent  # Resolve Thesis_code directory dynamically
        path = code_dir / path
        if not path.is_absolute():
            logging.error(f"Failed to resolve absolute path for model file: {model_path}")
            raise ValueError(f"Failed to resolve absolute path for model file: {model_path}")
    if not path.is_file():
    ...
    if not path.suffix == '.pth':
        ...
        raise ValueError(f"Invalid model file format: {model_path}. Expected a .pth file.")
    if not path.is_absolute():
