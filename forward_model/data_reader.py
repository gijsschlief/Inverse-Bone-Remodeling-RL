...
def forward_data_reader(
    ...
    if not isinstance(file_path, (str, Path, list)):
        logging.error(f"Invalid file path type: {type(file_path)}. Expected str, Path, or List[str].")
        return None
    if isinstance(file_path, list):
        if not all(isinstance(fp, (str, Path)) for fp in file_path):
            ...
            return None
        file_path = [Path(fp) if not isinstance(fp, Path) else fp for fp in file_path]
        return  _forward_data_load_multiple(file_path)
    ...
    logging.error(f"Invalid file path: {file_path}. It must be a file or a directory containing JSON files.")
    return None

def _forward_data_load_multiple(file_paths: Sequence[Path]) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
    """
...
def _convert_forward_data_to_numpy(data: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for error_msg, count in error_counts.items():
        logging.warning(f"Skipped {count} malformed entr{'y' if count == 1 else 'ies'} (e.g., {error_msg})")
...
if __name__ == "__main__":
    _, force_profiles, output_densities = forward_data_reader("/home/gijs/Desktop/Thesis/data/raw/")
