...
def test_invalid_ratios_raise() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="sum of train_data_ratio and validation_data_ratio must be less than 1",
    ):
    ...
    with pytest.raises(
        ValueError,
        match="train_data_ratio and validation_data_ratio must be convertible to float",
    ):
...
def test_mismatched_lengths_raise() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Input features and output features must have the same number of samples",
    ):
        splitting(x, y2)
