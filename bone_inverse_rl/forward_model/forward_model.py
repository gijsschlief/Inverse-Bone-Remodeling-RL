...
def forward_model(force_profile, initial_density, time_steps, dt, parameters):
    """
    ...
    Parameters:
        ...
        parameters (dict): Parameters for the forward model, including:
            - 'alpha': Rate of change of bone density.
            - 'beta': Rate of change of force profile.
    ...
    N = parameters.get('N', 40)
    rho0 = parameters.get('rho0', 0.8)
    rho_min = parameters.get('rho_min', 0.01)
    rho_max = parameters.get('rho_max', 1.74)
    k = parameters.get('k', 0.25)  # J/g
    B = parameters.get('B', 1)
    nu = parameters.get('nu', 0.3)
    M = parameters.get('M', 100)
    gamma = parameters.get('gamma', 2.0)
    T = parameters.get('T', 100)
    dt = parameters.get('dt', 1)
    ...
    cnt_cell_converged = []
    FName_str = str(parameters['file_location'])
    rho_val = []
    ...
    return updated_rho_val, cnt_cell_converged, t

if __name__ == "__main__":
    force_profile = np.random.rand(40, 40)  # Example force profile
    ...
    updated_density, converged_cells, final_time = forward_model(force_profile, initial_density, time_steps, dt, parameters)
    print("Updated Density:", updated_density)
    print("Converged Cells:", converged_cells)
